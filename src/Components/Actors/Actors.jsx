...
import { fetchActorsOfMovie } from '../../utils/api'
...
const navigateHandler=(actorid,actorname)=>{
    navigate('/person/'+actorname,{state:actorid})
}
    useEffect(() => {
        const getCredits = async () => {
            try {
                const response = await fetchActorsOfMovie(movieType, movieId)
    ...
    return (
        <div className=' flex  mx-2 overflow-x-scroll scrollbar-hide text-white'>
            {actors.map((actor) => (
                <div key={actor.id} className='p-1  ' onClick={()=>navigateHandler(actor.id,actor.name)}>
